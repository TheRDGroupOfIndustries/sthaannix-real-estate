...
export const uploadPaymentProof = async (req: Request, res: Response) => {
  ...
  const uploaded = await uploadFile(req.files.screenshot[0].buffer, "payments/proofs");
  screenshot = uploaded.secure_url;
} else if (req.files?.proof?.[0]) {
  const uploaded = await uploadFile(req.files.proof[0].buffer, "payments/proofs");
  screenshot = uploaded.secure_url;
} else if (req.body.proof) {
  screenshot = req.body.proof;
} else {
  return res.status(400).json({ message: "Proof image required (file or URL)" });
}
