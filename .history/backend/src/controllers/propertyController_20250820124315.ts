...
export const createProperty = async (req: Request, res: Response) => {
  try {
    ...
    const {
      ...
      location,
      isPromoted,
      imageUrls, // <-- coming from req.body
    } = req.body;

    const uploadedImages: string[] = [];

    // 1. If files are uploaded, upload them to storage (e.g., Cloudinary)
    if (req.files && Array.isArray(req.files)) {
      for (const file of req.files as Express.Multer.File[]) {
        const result = await uploadFile(file.buffer, "properties/images");
        uploadedImages.push(result.secure_url);
    ...
    // 2. If image URLs are provided in body, add them
    ...
    const property = new Property({
      ...
      isPromoted,
      owner: req.user.id,
      images: uploadedImages, // both uploaded & URL images
      videos: [],
    });
