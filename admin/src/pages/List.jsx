...
import { Link } from "react-router-dom";
import { motion, AnimatePresence } from "framer-motion";
import { propertiesAPI } from "../api/api"; // Use your centralized API
...
const PropertyListings = () => {
  ...
  const filteredProperties = properties
    .filter((property) => {
      const matchesSearch =
        !searchTerm ||
        [property.title, property.type, property.location?.city, property.location?.state].some((field) =>
  ...
  if (loading) {
    return (
      <motion.div className="min-h-screen pt-20 flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100">
        <div className="text-center">
          <motion.div
            animate={{ rotate: 360 }}
            transition={{ duration: 1, repeat: Infinity, ease: "linear" }}
          ...
          <p className="text-gray-500">Fetching the latest property listings...</p>
        </div>
      </motion.div>
    );
  }
    ...
    <motion.div variants={containerVariants} initial="hidden" animate="visible" className="min-h-screen pt-20 bg-gradient-to-br from-gray-50 via-white to-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-6 mb-8">
          <div>
            <h1 className="text-4xl font-bold bg-gradient-to-r from-gray-900 to-gray-600 bg-clip-text text-transparent">Property Management</h1>
        ...
        {/* Property Grid/List */}
        <div className={`space-y-6 ${viewMode === "grid" ? "grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6" : "space-y-4"}`}>
          <AnimatePresence>
            {filteredProperties.length === 0 ? (
              ...
              filteredProperties.map((property, index) => (
                ...
                >
                  <img
                    src={property.images?.[0] || "https://dummyimage.com/600x400/000/fff"}
                    alt={property.title}
                  ...
                  <div className="p-6">
                    ...
                    <p className="text-gray-900 font-bold text-lg mb-2">₹{property.price.toLocaleString()}</p>
                    <div className="flex items-center gap-2">
                      <button onClick={() => handleRemoveProperty(property._id, property.title)} className="p-2 text-red-600 hover:text-white hover:bg-red-600 rounded-full transition">
                        <Trash2 className="w-4 h-4" />
                      </button>
                      <Link to={`/update/${property._id}`} className="p-2 text-blue-600 hover:text-white hover:bg-blue-600 rounded-full transition">
                        <Edit3 className="w-4 h-4" />
                      </Link>
                    </div>
                  </div>
