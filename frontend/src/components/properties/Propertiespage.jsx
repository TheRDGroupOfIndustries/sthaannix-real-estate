...
const PropertiesPage = () => {
  ...
  const [propertyState, setPropertyState] = useState({
    properties: getLocalStorage(LOCAL_STORAGE_KEY) || [],
    loading: false,
    error: null,
  ...
  const fetchPropertiesHandler = async () => {
    try {
      ...
      // To simulate no backend: just use localStorage contents, no API call
      setPropertyState((prev) => ({
        ...prev,
        loading: false,
      }));
  ...
  useEffect(() => {
    if (propertyState.properties && propertyState.properties.length > 0) {
      setLocalStorage(LOCAL_STORAGE_KEY, propertyState.properties);
    }
